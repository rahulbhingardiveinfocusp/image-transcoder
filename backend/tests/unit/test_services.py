...
async def test_get_upload_url_creates_record_and_returns_url():
    ...
    with patch(
        ...
        result = await ImageService.get_upload_url(repo, "photo.jpg", "image/jpeg")
...
async def test_process_image_copies_and_deletes_s3_object():
    ...
    repo = make_mock_repo(list_all=AsyncMock(return_value=[existing_item]))
    ...
    with patch.object(ImageService, "_get_s3_client", return_value=mock_s3):
        new_key = await ImageService.process_image(repo, "my-bucket", "raw/abc-123-photo.jpg")

    assert new_key == "processed/abc-123-photo.jpg"
    mock_s3.copy_object.assert_called_once_with(
        Bucket="my-bucket",
        CopySource={"Bucket": "my-bucket", "Key": "raw/abc-123-photo.jpg"},
        Key="processed/abc-123-photo.jpg",
    )
    mock_s3.delete_object.assert_called_once_with(
        Bucket="my-bucket", Key="raw/abc-123-photo.jpg"
    )


@pytest.mark.asyncio
async def test_process_image_raises_when_record_not_found():
    repo = make_mock_repo(list_all=AsyncMock(return_value=[]))

    with pytest.raises(ValueError, match="Record not found"):
        await ImageService.process_image(repo, "my-bucket", "raw/missing.jpg")
...
async def test_get_all_images_maps_dynamo_items_correctly():
    ...
    with patch(
        ...
        result = await ImageService.get_all_images(repo)
